  ...
  Future<LoginResponse?> signInWithGoogle(BuildContext context) async {
    ...
    if (googleSignInAccount != null) {
      ...
      UserData tempUserData = UserData()
        ...
        ..uid = user.uid
        ..username = (currentUser.email.validate().splitBefore('@').replaceAll(
            '.', '')).toLowerCase();
      return await loginCurrentUsers(
          context, req: tempUserData.toJson(), isSocialLogin: true);
    } else {
...
//endregion
  ...
  Future<String> signUpWithEmailPassword(BuildContext context,
    ...
    return await _auth.createUserWithEmailAndPassword(
        email: userData.email.validate(), password: DEFAULT_FIREBASE_PASSWORD)
  ...
  Future<String> signInWithEmailPassword(
    ...
    return await _auth.signInWithEmailAndPassword(
        email: email, password: DEFAULT_FIREBASE_PASSWORD).then((value) async {
      return value.user!.uid.validate();
    }).catchError((e) async {
      if (e.code == 'user-not-found') {
        await registerInWithEmailPassword(
            email: email, isSocialLogin: false, uid: "");
      }
      appStore.setLoading(false);
      log(e.toString());
  ...
  Future<String> registerInWithEmailPassword(
      {required String email, String? uid, bool isSocialLogin = false}) async {
    if (isSocialLogin) {
      return uid.validate();
    }
  ...
  Future<String> setRegisterData({required UserData userData}) async {
    return await userService.addDocumentWithCustomId(
        userData.uid.validate(), userData.toFirebaseJson()).then((value) async {
  ...
  Future loginWithOTP(BuildContext context,
      {String phoneNumber = "", String? countryCode, String? countryISOCode}) async {
          ...
          onTap: (otpCode) async {
            if (otpCode != null) {
              ...
              await _auth.signInWithCredential(credential).then((
                ...
                await loginCurrentUsers(
                    context, req: request, isSocialLogin: true).then((
                    loginResponse) async {
                  log("=============================== ${loginResponse
                      .toJson()}");
                  ...
                  if (loginResponse.isUserExist == null) {
                    if (loginResponse.userData!.status == 0) {
                      ...
                      loginResponse.userData!.uid =
                          credentials.user!.uid.validate();
                      saveDataToPreference(
                          context, userData: loginResponse.userData!,
                          isSocialLogin: true,
                          onRedirectionClick: () {
                    ...
                    appStore.setLoading(false);
                    finish(context);
                    SignUpScreen(isOTPLogin: true,
                        phoneNumber: phoneNumber,
                        countryCode: countryISOCode,
                        uid: credentials.user!.uid.validate()).launch(context);
                  }
                }).catchError((e) {
                  appStore.setLoading(false);
                  toast(e.toString(), print: true);
...
//endregion
  Future<void> loginFromFirebaseUser(User currentUser,
      {LoginResponse? loginData, String? displayName, String? loginType}) async {
    if (await userService.isUserExist(loginData!.userData!.email)) {
      log("Firebase User Exist");

      await userService.userByEmail(loginData.userData!.email).then((
          user) async {
      ...
      await userService.addDocumentWithCustomId(
          currentUser.uid.validate(), loginData.userData!.toJson()).then((
          value) async {
    ...
    if (await TheAppleSignIn.isAvailable()) {
      ...
      switch (result.status) {
        ...
        case AuthorizationStatus.error:
          throw ("${language.lblSignInFailed}: ${result.error!
  ...
  Future<void> saveAppleDataWithoutEmail(User user) async {
    ...
    var req = {
      'email': getStringAsync(APPLE_EMAIL).isNotEmpty ? getStringAsync(
          APPLE_EMAIL) : getStringAsync(APPLE_UID) + '@gmail.com',
      'first_name': getStringAsync(APPLE_GIVE_NAME),
      'last_name': getStringAsync(APPLE_FAMILY_NAME),
      "username": getStringAsync(APPLE_EMAIL).isNotEmpty ? getStringAsync(
          APPLE_EMAIL) : getStringAsync(APPLE_UID) + '@gmail.com',
      "profile_image": '',
      "social_image": '',
    ...
    await loginUser(req, isSocialLogin: true).then((value) async {
      await loginFromFirebaseUser(user, loginData: value,
