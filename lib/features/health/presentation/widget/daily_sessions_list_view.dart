...
import 'package:starter_application/features/health/presentation/widget/daily_session_item_card.dart';
import 'package:starter_application/features/health/presentation/widget/health_item_card.dart';

class DailySessionsListView extends StatelessWidget {
  final double itemHeight;
  final String title;
  final String? icon;
  final List<DailySessionItemEntity> items;
  ...
  Widget _buildHealthItemCard(int index) {
    ...
    return DailySessionItemCard(
      height: itemHeight,
      name: '${item.session.title}',
      image: '${item.session.imageUrl}',
      amountOfCalories: '${item.session.amountOfCalories}',
      timeInMinute: '${item.session.timeInMinutes}',
    );
  }
}
