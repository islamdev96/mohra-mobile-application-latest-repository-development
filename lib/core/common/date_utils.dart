...
class DateUtility {
  ...
  static String currentDateAsString(){
    ...
    return _apiDayFormat.format(now);
  }
  static DateTime todayDate(){
    return parseDateStringToFormat(currentDateAsString());
  }

  static DateTime parseDateStringToFormat(String inputDate){
    return _apiDayFormat.parse(inputDate);
  }
  static Duration differenceBetweenTwoDays(DateTime currentTime , DateTime toDate){
    return currentTime.difference(toDate) ;
  }
}
...
class DateUtils {
  ...
  static getNewDateDependsOnRepeatMode(DateTime dateTime, int repeatMode , int? clientId){
    if (clientId != null){
      return dateTime;
    }
    else{
      if(repeatMode == 2){
        ...
        // monthly
        DateTime dateTime2 = DateTime(dateTime.year ,DateTime.now().month+1,dateTime.day,dateTime.hour,dateTime.minute, );
        return dateTime2;
      }
        ...
        if(dateTime.compareTo(now)> 0){
          ...
          return DateTime(DateTime.now().year +1,dateTime.month,dateTime.day,dateTime.hour,dateTime.minute, );
        }
      }
    }
  }
